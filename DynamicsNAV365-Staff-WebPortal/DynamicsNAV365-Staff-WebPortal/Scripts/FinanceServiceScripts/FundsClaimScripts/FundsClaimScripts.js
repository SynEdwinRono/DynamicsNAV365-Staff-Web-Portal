...
function AddDropDownListSearch() {
	$("#ServiceProvider").select2({
		placeholder: $("#ServiceProviderLbl").text(),
		allowClear: true
	});

	$("#FundsClaimCode").select2({
		placeholder: $("#FundsClaimCodeLbl").text(),
		allowClear: true
	});
}
	...
	$.ajax({
		...
		success: function (fundsClaimLines) {
			...
			$.each(fundsClaimLines, function (i, fundsClaimLine) {
				rows += "<tr>";
				rows += "<td>" + fundsClaimLine.FundsClaimCode + "</td>";
				rows += "<td>" + fundsClaimLine.LineDescription + "</td>";
				rows += "<td>" + fundsClaimLine.LineAmount.toLocaleString() + "</td>";
				rows += '<td><a href="javascript:void(0);" onclick="return EditFundsClaimLine(' + fundsClaimLine.LineNo + ',\'' + fundsClaimLine.DocumentNo + '\');"><i class="fa fa-edit" aria-hidden="true">Edit</i></a> | <a href="#" onclick="DeleteFundsClaimLine(' + fundsClaimLine.LineNo + ',\'' + fundsClaimLine.DocumentNo + '\')"><i class="fa fa-trash" aria-hidden="true">Delete</i></a></td>';
...
function CreateFundsClaimLine() {
	...
	var FundsClaimLineObj = {
		...
		FundsClaimCode: $("#FundsClaimCode").val(),
...
function EditFundsClaimLine(LineNo, DocumentNo) {
	$.ajax({
		...
		success: function (result) {
			...
			$('#FundsClaimCode').val(result.FundsClaimCode).trigger("change");
...
function ValidateFundsClaimLine() {
	var isValid = true;
	var label = "";
	if ($("#FundsClaimCode").val().trim() == "") {
		$("#FundsClaimCodeError").text("Funds code cannot be empty.");
		isValid = false;
	}
	else {
...
function ClearFundsClaimLineModal() {
	$("#LineNo").val(0);
	$("#DocumentNo").val("");
	$("#FundsClaimCode").val("").trigger("change");
	$("#LineDescription").val("");
	$("#LineAmount").val(0);
